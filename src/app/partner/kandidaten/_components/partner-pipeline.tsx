...
export function PartnerPipelineBoard({ placements }: { placements: PlacementData[] }) {
  ...
  const [selectedStage, setSelectedStage] = useState<PlacementStage>(
    PLACEMENT_PIPELINE_STAGES[0]
  );
  ...
  const mobileStages: PlacementStage[] = rejected.length
    ? [...activeStages, "abgelehnt"]
    : [...activeStages];

  function renderRejectedCard(p: PlacementData) {
  ...
  function renderCard(p: PlacementData, stage: PlacementStage) {
    const c = p.candidates;
    if (!c) return null;
    const isAnonymized = stage === "leadeingang";
    const nextStage = activeStages[activeStages.indexOf(stage) + 1] as
      | PlacementStage
      | undefined;
        ...
        <div className="p-4">
          ...
          {!isAnonymized && (
            <div className="mt-3 space-y-1 border-t pt-3">
              <a href={`mailto:${c.email}`} className="flex min-h-8 items-center gap-1.5 text-xs text-primary hover:underline">
                <Mail className="h-3 w-3" />
                {c.email}
              </a>
              {c.telefon && (
                <a href={`tel:${c.telefon}`} className="flex min-h-8 items-center gap-1.5 text-xs text-primary hover:underline">
        ...
        {stage !== "eingestellt" && (
          <div className="flex items-center gap-2 border-t bg-muted/30 px-4 py-2.5">
            {nextStage && (
              <Button
                size="sm"
                className="min-h-10 flex-1 gap-1.5 text-xs md:min-h-8"
            ...
            <Button
              size="sm"
              variant="ghost"
              className="min-h-10 text-xs text-destructive hover:text-destructive md:min-h-8"
  ...
  const mobileItems = items.filter((p) => p.status === selectedStage);
      ...
      <div className="md:hidden">
        <div className="-mx-4 mb-4 flex gap-2 overflow-x-auto px-4 pb-1">
          {mobileStages.map((stage) => {
            const count = items.filter((p) => p.status === stage).length;
        ...
        {mobileItems.length === 0 ? (
          ...
          <div className="space-y-3">
            {mobileItems.map((p) =>
              selectedStage === "abgelehnt"
                ? renderRejectedCard(p)
                : renderCard(p, selectedStage)
            )}
          </div>
        )}
      </div>
      ...
      <div className="hidden gap-4 overflow-x-auto pb-4 md:flex">
          ...
          return (
            <div key={stage} className="w-72 shrink-0">
              ...
              <div className="space-y-3">
                ...
                {stageItems.map((p) => renderCard(p, stage))}
              </div>
            </div>
          );
        })}
        ...
        {rejected.length > 0 && (
          <div className="w-72 shrink-0">
            ...
            <div className="space-y-3">
              {rejected.map((p) => renderRejectedCard(p))}
            </div>
          </div>
        )}
      </div>
